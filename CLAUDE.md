...
**Important:** Always run `npx playwright test` from `frontend/` after making frontend changes. All 47 tests must pass before considering work complete.
...
**Key flow:** Cognito User Pool authenticates users → JWT ID token sent in `Authorization: Bearer` header → API Gateway (HTTP API) validates JWT via Cognito authorizer → Routes to Lambda functions → Lambda reads/writes DynamoDB (movies table for catalog, queue table for per-user lists). Username is extracted from path parameters or JWT claims.
...
- **api_gateway.tf** - HTTP API (API Gateway v2) with Cognito JWT authorizer, `dev` stage with auto-deploy, 8 routes mapping to 4 Lambda integrations, and Lambda invoke permissions for API Gateway
...
- **config.tf** - Generates `config.json` for the frontend with Cognito IDs and `apiBaseUrl` (API Gateway endpoint)
- **locals.tf** - Naming convention: `{project_name}-{environment}-*` with common tags
- **outputs.tf** - Exports Cognito IDs, S3 endpoint, Lambda function names/ARNs, DynamoDB table names (movies + queue), API Gateway URL, region
...
- **movieq_catalog** (`lambdas/movieq_catalog/handler.py`) - Handles two methods:
  - `GET /movies` — extracts `username` from JWT claims (`cognito:username`), scans the full movies table catalog and queries the user's queue to build a queued status map. Returns `{"movies": [...], "queued": {"movie_id": "status", ...}}`. Used by the "All Movies" catalog view.
  - `DELETE /movies/{movie_id}` — deletes a movie from the shared catalog. First validates that no users have the movie queued by scanning the queue table; returns `409` with `{"error": "...", "queued_by": ["user1", ...]}` if found. Returns `404` if movie doesn't exist. On success, deletes from movies table and returns `200`.

### Frontend (`frontend/`)

- React + Vite + Tailwind CSS
- `awsClients.js` - Cognito auth flow (returns `username` + `idToken` from login), and `apiCall(baseUrl, idToken, method, path, body?)` helper using `fetch()` with JWT `Authorization: Bearer` header
...
- `CatalogRow.jsx` - Catalog row component for "All Movies" view: shows movie details, colored badge if already queued, "Add to..." dropdown (includes list options and "Delete from Catalog" with confirmation popover), and kebab menu with "Edit" option
...
- `MovieForm.jsx` - Add/Edit movie form with validation (only `title` + `year` required; `genre`, `rating`, `director` optional). Includes client-side duplicate detection (>=90% title similarity + same year) before save. Accepts `hideStatus` prop to hide the status dropdown when editing from catalog view
...
- `MovieRow.jsx` - Two exports: `MovieRowContent` (presentational, used by both sortable rows and DragOverlay) and `SortableMovieRow` (default, wraps `useSortable` hook). Drag handle (6-dot grip), kebab menu (Move to Top/Bottom, Remove from List), status dropdown. Edit is only available from the catalog view, not from queue list rows
...
- Ratings are always displayed with one decimal place (e.g. `8.0`, `9.2`) using `Number(rating).toFixed(1)`
- All pages use the `cinema-background-heavy.jpg` background image

### Playwright Tests (`frontend/tests/`)

- Tests run against local Vite dev server (port 8080) with **all AWS calls mocked** (Cognito, API Gateway)
- Mock setup in `tests/helpers.js` — intercepts `/config.json`, Cognito auth, and API Gateway REST routes
...
All Lambda functions are accessed via an HTTP API (API Gateway v2) with Cognito JWT authorization.
...
| `DELETE` | `/movies/{movie_id}` | `movieq_catalog` | Delete movie from catalog (fails if queued by any user) |
